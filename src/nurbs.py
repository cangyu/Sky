...
from src.iges import Entity110, Entity126, Entity128
...
def normalize(vec):
    ...
    tmp = norm(vec, 2)
    return vec if math.isclose(tmp, 0.) else vec / tmp
...
def line_intersection(p1, u1, p2, u2, with_ratio=False):
    ...
    if len(p1) != len(u1) != len(p2) != len(u2):
    ...
    dp = p2 - p1
    if not np.cross(u1, u2).any():
        err_msg = "Two lines are parallel." if np.cross(dp, u1).any() else "Two lines coincide with each other."
        raise AssertionError(err_msg)

    nu1 = normalize(u1)
    nu2 = normalize(u2)
    vu1 = u2 - np.dot(nu1, u2) * nu1
    vu2 = u1 - np.dot(nu2, u1) * nu2
    alpha1 = np.dot(dp, vu2) / np.dot(u1, vu2)
    alpha2 = -np.dot(dp, vu1) / np.dot(u2, vu1)
    pans1 = p1 + alpha1 * u1
    pans2 = p2 + alpha2 * u2
